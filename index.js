/* ============================================================
   STUDIOBLOX v1.0.0 - PROFESYONEL DISCORD BOTU
   Tek dosya: index.js
   Kurulum: CONFIG bölümünü doldur -> npm install -> node index.js
   ============================================================ */
const {
...
/* ========================= CONFIG ========================= */
const CONFIG = {
  token: "BURAYA_BOT_TOKENINI_YAPISTIR",          // Developer Portal > Bot > Reset Token
  ownerId: "BURAYA_KENDI_DISCORD_IDNI_YAZ",       // Developer Mode açıkken kendine sağ tık > Copy ID
  mequeenBanner: "BURAYA_MEQUEEN_BANNER_GORSEL_URLSI", // 2. fotoğrafın direkt linki (rehbere bak)
  version: "1.0.0"
};
/* ========================================================== */
...
catch (e) { DB = { guilds: {}, users: {}, giveaways: {}, tickets: {}, apps: {} }; }
...
async function dmUser(user, payload) {
  try { await user.send(payload); return true; } catch (e) { return false; }
}
async function modlog(guild, embed) {
  const c = gconf(guild.id).modlog;
  if (!c) return;
  const ch = guild.channels.cache.get(c);
  if (ch) try { await ch.send({ embeds: [embed] }); } catch (e) {}
}
function adminCheck(i) {
  if (isOwner(i.user)) return true;
  return i.member && i.member.permissions.has(PermissionFlagsBits.Administrator);
}
function modCheck(i) {
  if (isOwner(i.user)) return true;
  return i.member && (i.member.permissions.has(PermissionFlagsBits.ModerateMembers) || i.member.permissions.has(PermissionFlagsBits.Administrator));
}

/* Geçici kurulum durumları */
const STATE = new Map();
const sKey = (i, extra = '') => `${i.guild ? i.guild.id : 'dm'}:${i.user.id}${extra}`;
setInterval(() => {
  for (const [k, v] of STATE) if (v.exp && v.exp < now()) STATE.delete(k);
}, 300000);
...
/* ---------- TICKET KUR ---------- */
CMDS.push({
  data: new SlashCommandBuilder().setName('ticket-kur')
    .setDescription('🛠️ Kategori seçmeli gelişmiş ticket sistemini kurar (panel ayarlı)')
  ...
  async execute(i) {
    ...
    await i.reply({
      embeds: [E().setTitle('🛠️ Ticket Sistemi Kurulumu').setDescription(
...
/* ---------- TICKET KUR MEQUEEN ---------- */
CMDS.push({
  data: new SlashCommandBuilder().setName('ticket-kur-mequeen')
    .setDescription('🛠️ Mequeen Studio için hazır ticket panelini kurar')
    .setDMPermission(false),
  async execute(i) {
    if (!adminCheck(i)) return i.reply({ embeds: [ERR('Bu komutu kullanmak için **Yönetici** olmalısın.')], ephemeral: true });
    STATE.set(sKey(i) + ':mq', { exp: now() + 600000 });
    await i.reply({
      embeds: [E().setTitle('🛠️ Mequeen Studio Ticket Kurulumu').setDescription(
        '> Hazır şablon: **Mequeen Studio Destek & Support**\n\n`1.` Panelin atılacağı kanalı seç\n`2.` Ticket açılınca etiketlenecek rolleri seç (isteğe bağlı)\n`3.` **Paneli Kur** butonuna bas\n\nKategoriler: `Bug Bildir` `Şikayet` `Yetkili Alım` `Diğer`')],
      components: [
...
/* ---------- MODERASYON ---------- */
CMDS.push({
  data: new SlashCommandBuilder().setName('ban').setDescription('🔒 Üyeyi sunucudan yasaklar').setDMPermission(false)
    ...
    .addIntegerOption(o => o.setName('mesaj_sil').setDescription('Son X saniye mesajlarını sil').setRequired(false)),
  async execute(i) {
...
CMDS.push({
  data: new SlashCommandBuilder().setName('kick').setDescription('🔒 Üyeyi sunucudan atar').setDMPermission(false)
...
CMDS.push({
  data: new SlashCommandBuilder().setName('mute').setDescription('🔒 Üyeyi süreli susturur (timeout)').setDMPermission(false)
...
CMDS.push({
  data: new SlashCommandBuilder().setName('uyarilar').setDescription('🔒 Üyenin uyarılarını listeler').setDMPermission(false)
    .addUserOption(o => o.setName('uye').setDescription('Üye').setRequired(true)),
  async execute(i) {
    if (!modCheck(i)) return i.reply({ embeds: [ERR('Yetkin yok.')], ephemeral: true });
    const m = i.options.getMember('uye');
    const list = gconf(i.guild.id).warns[m.id] || [];
    const eb = E().setTitle('🔒 Uyarı Listesi').setDescription(list.length
      ? list.map((w, x) => `**${x + 1}.** ${w.reason}\n-# <t:${Math.floor(w.time / 1000)}:f> • <@${w.by}>`).join('\n\n')
      : '> Bu üyenin uyarısı yok.');
    i.reply({ embeds: [eb], ephemeral: true });
  }
});
CMDS.push({
  data: new SlashCommandBuilder().setName('uyari-sil').setDescription('🔒 Uyarı siler (numara ile)').setDMPermission(false)
    .addUserOption(o => o.setName('uye').setDescription('Üye').setRequired(true))
...
CMDS.push({
  data: new SlashCommandBuilder().setName('slowmode').setDescription('⚙️ Kanal yavaş modunu ayarlar').setDMPermission(false)
...
CMDS.push({
  data: new SlashCommandBuilder().setName('kilitle').setDescription('🔒 Kanalı herkese yazmaya kapatır').setDMPermission(false)
...
CMDS.push({
  data: new SlashCommandBuilder().setName('otorol').setDescription('🏷️ Yeni üyelere otomatik rol ayarlar').setDMPermission(false)
  ...
  async execute(i) {
    ...
    if (i.options.getSubcommand() === 'kur') {
      ...
      i.reply({ embeds: [OKC(`Otorol kuruldu: <@&${g.autorole}>\n> Yeni üyelerine otomatik verilecek.`)] });
    } else {
      g.autorole = null;
      i.reply({ embeds: [OKC('Otorol kapatıldı.')] });
    }
  }
});
CMDS.push({
  data: new SlashCommandBuilder().setName('guard').setDescription('⚙️ Guard filtrelerini yönetir').setDMPermission(false)
    ...
    .addSubcommand(s => s.setName('reklam').setDescription('Discord davet linki filtresi')
      .addStringOption(o => o.setName('durum').setDescription('aç/kapat').setRequired(true).addChoices({ name: 'Aç', value: 'ac' }, { name: 'Kapat', value: 'kapat' }))),
  async execute(i) {
    if (!adminCheck(i)) return i.reply({ embeds: [ERR('Yönetici olmalısın.')], ephemeral: true });
    const g = gconf(i.guild.id);
...
duyuruCmd('güncelleme-duyuru', '📢 Profesyonel güncelleme duyurusu oluşturur', 'guncelleme');
duyuruCmd('leak-duyuru', '📢 Profesyonel leak/sızıntı duyurusu oluşturur', 'leak');
...
function annEmbed(a, user) {
  ...
  const eb = E(color);
  if (a.type === 'leak') {
    eb.setDescription(`**👀 LEAK / SIZINTI BÜLTENİ**\n> **${a.baslik}**\n> *Kaynak doğrulanmamıştır; paylaşım sorumluluğu kullanıcıya aittir.*\n\n${a.icerik}`);
  } else {
    eb.setDescription(`**✨ GÜNCELLEME DUYURUSU**\n> **${a.baslik}**\n\n${a.icerik}`);
  }
...
CMDS.push({
  data: new SlashCommandBuilder().setName('çekiliş').setDescription('🎉 Gelişmiş çekiliş sistemi').setDMPermission(false)
    .addSubcommand(s => s.setName('başlat').setDescription('Çekiliş başlatır')
      ...
      .addStringOption(o => o.setName('tur').setDescription('Çekiliş türü').setRequired(true).addChoices({ name: 'Normal', value: 'normal' }, { name: 'Rol Şartlı', value: 'rol' }, { name: 'Seviye/Emek Şartlı', value: 'emek' }))
      .addRoleOption(o => o.setName('rol').setDescription('Rol şartı (tür=rol ise)'))
      ...
      .addRoleOption(o => o.setName('ping_rol').setDescription('Ping rolü (ping=rol ise)'))
      .addStringOption(o => o.setName('aciklama').setDescription('Ek açıklama')))
    .addSubcommand(s => s.setName('bitir').setDescription('Çekilişi erken bitirir').addStringOption(o => o.setName('mesaj_id').setDescription('Çekiliş mesaj ID').setRequired(true)))
    .addSubcommand(s => s.setName('yeniden').setDescription('Yeniden kazanan çeker').addStringOption(o => o.setName('mesaj_id').setDescription('Çekiliş mesaj ID').setRequired(true))),
  async execute(i) {
    if (!modCheck(i)) return i.reply({ embeds: [ERR('Yetkin yok.')], ephemeral: true });
    const sub = i.options.getSubcommand();
    if (sub === 'başlat') {
      ...
      const ch = i.options.getChannel('kanal') || i.channel;
      const g = {
        gid: i.guild.id, cid: ch.id, mid: null,
        end: now() + ms, winners: i.options.getInteger('kazanan'),
        prize: i.options.getString('odul'), type: i.options.getString('tur'),
        roleReq: i.options.getRole('rol') ? i.options.getRole('rol').id : null,
        ping: i.options.getString('ping') || 'yok',
        pingRole: i.options.getRole('ping_rol') ? i.options.getRole('ping_rol').id : null,
        desc: i.options.getString('aciklama') || null,
        parts: [], rem6: false, rem1: false, ended: false, sponsor: i.user.id
      };
    ...
    if (sub === 'bitir' || sub === 'yeniden') {
      const id = i.options.getString('mesaj_id');
...
function gwEmbed(g) {
  ...
  const tur = g.type === 'rol' ? 'Rol Şartlı' : g.type === 'emek' ? 'Emek/Seviye Şartlı' : 'Normal';
...
async function endGiveaway(g, reroll) {
  ...
  try {
    const msg = await ch.messages.fetch(g.mid);
    await msg.edit({ embeds: [eb], components: [] });
  } catch (e) {}
  if (wins.length) {
    ...
    await ch.send({ content: `**🎉 Çekiliş sona erdi:** yeterli katılımcı yok, kazanan yok.` });
  }
  saveDB();
}
...
CMDS.push({
  ...
  async execute(i) {
    ...
    await i.reply({
      embeds: [E().setTitle('📋 Başvuru Sistemi Kurulumu').setDescription(
        '`1.` Panel & log kanalını, kabul rolünü seç\n`2.` **Panel Metni** ile embed yazısını ayarla\n`3.` **Soru Ekle** ile soruları ekle (en fazla 15)\n`4.` **Sistemi Kur** ile bitir')],
      components: [
...
/* ---------- DM AT ---------- */
CMDS.push({
  data: new SlashCommandBuilder().setName('dm-at').setDescription('📢 Üyelere DM üzerinden mesaj gönderir').setDMPermission(false),
  async execute(i) {
    if (!adminCheck(i)) return i.reply({ embeds: [ERR('Yönetici olmalısın.')], ephemeral: true });
    STATE.set(sKey(i) + ':dm', { exp: now() + 600000, target: null, member: null });
    await i.reply({
      embeds: [E().setTitle('📢 DM Gönderim Sistemi').setDescription('> Kime gönderileceğini seç:\n`everyone` → tüm üyeler\n`here` → aktif (çevrimiçi) üyeler\n`uye` → tek üye')],
      components: [new ActionRowBuilder().addComponents(
...
const CATS = {
  ...
  ticket: [['/ticket-kur', 'Kategorili/kategorisiz ticket paneli kurar'], ['/ticket-kur-mequeen', 'Mequeen Studio hazır paneli'], ['Panel menüsü', 'Kategori seçerek ticket açar'], ['Ticket butonları', 'Kapat + otomatik transcript']],
  duyuru: [['/güncelleme-duyuru', 'Markdown destekli güncelleme duyurusu'], ['/leak-duyuru', 'Sızıntı bülteni'], ['Önizleme', 'Göndermeden embed önizleme + kanal/mention seçimi']],
  cekilis: [['/çekiliş başlat', 'Süre, ödül, tür, rol şartı, ping ayarlı'], ['/çekiliş bitir', 'Erken bitirir'], ['/çekiliş yeniden', 'Reroll'], ['Otomatik', '6 saat & 1 saat kala otomatik hatırlatma pingi']],
  ...
  dm: [['/dm-at', 'Everyone / Here / Tek üye DM gönderimi']],
  ekonomi: [['/robux bakiye', 'Cüzdan + banka + envanter'], ['/robux cash', 'Üyeler arası transfer'], ['/robux günlük', '24 saatte bir R$'], ['/robux çalış', 'İşlerden kazanç'], ['/robux ara', 'Rastgele R$ bul'], ['/robux yazıtura', 'Bot ile coinflip'], ['/robux bahis', 'Zar bahsi (x2 / x10)'], ['/robux duel', 'Üye ile düello'], ['/robux yatır / çek', 'Banka'], ['/robux market', 'Mağaza'], ['/robux envanter', 'Eşyalar'], ['/robux liderler', 'Top 10'], ['/robux bilgi', 'Rehber']],
  genel: [['/yardım', 'Bu menü (DM)'), ['/ping', 'Bot gecikmesi'], ['/avatar', 'Profil fotoğrafı'], ['/sunucu-bilgi', 'Sunucu istatistiği'], ['/üye-bilgi', 'Üye kartı'], ['/user-count', 'Kilitli sayaç ses kanalları']]
};
function helpEmbed(cat) {
  if (!cat) {
    return E().setTitle('👋 Studioblox Yardım Merkezi')
      .setDescription(`> Aşağıdaki menüden kategori seç.\n\n**Kategoriler:**\n\`moderasyon\` \`ticket\` \`duyuru\` \`cekiliş\` \`basvuru\` \`dm\` \`ekonomi\` \`genel\`\n\n-# Bu mesaj DM üzerinden gönderildi.`);
  }
  const list = CATS[cat] || [];
  return E().setTitle(`👋 Yardım: ${cat.toUpperCase()}`)
    .setDescription(list.map(l => `**${l[0]}**\n> ${l[1]}`).join('\n\n'));
}
CMDS.push({
  data: new SlashCommandBuilder().setName('yardım').setDescription('👋 Yardım menüsünü DM üzerinden gönderir'),
  async execute(i) {
    ...
    else i.reply({ embeds: [ERR('DM kutun kapalı. Sunucu gizlilik ayarlarından **DM** izinlerini aç.')], ephemeral: true });
  }
});

/* ---------- USER COUNT ---------- */
CMDS.push({
  data: new SlashCommandBuilder().setName('user-count').setDescription('📊 Kilitli sayaç ses kanalları açar').setDMPermission(false)
    .addSubcommand(s => s.setName('kur').setDescription('Sayaç kanallarını kurar')
      .addStringOption(o => o.setName('dil').setDescription('Dil').setRequired(true).addChoices({ name: 'English', value: 'en' }, { name: 'Türkçe', value: 'tr' })))
    .addSubcommand(s => s.setName('kapat').setDescription('Sayaç kanallarını siler')),
  async execute(i) {
    if (!adminCheck(i)) return i.reply({ embeds: [ERR('Yönetici olmalısın.')], ephemeral: true });
    const g = gconf(i.guild.id);
    ...
    const cat = await i.guild.channels.create({
      name: lang === 'tr' ? '📊 İSTATİSTİK' : '📊 STATISTICS',
...
/* ---------- ROBUX (OWO-BENZERİ EKONOMİ) ---------- */
const rb = new SlashCommandBuilder().setName('robux').setDescription('💸 Robux ekonomi sistemi (OwO tarzı)')
  .addSubcommand(s => s.setName('bakiye').setDescription('Bakiye kartı').addUserOption(o => o.setName('uye').setDescription('Üye')))
  .addSubcommand(s => s.setName('cash').setDescription('Üyeye R$ gönderir').addUserOption(o => o.setName('uye').setDescription('Alıcı').setRequired(true)).addIntegerOption(o => o.setName('miktar').setDescription('Miktar').setRequired(true).setMinValue(1)))
  .addSubcommand(s => s.setName('günlük').setDescription('Günlük R$ ödülü'))
  .addSubcommand(s => s.setName('çalış').setDescription('İşten R$ kazan'))
  .addSubcommand(s => s.setName('ara').setDescription('Etrafı ara, R$ bul'))
  .addSubcommand(s => s.setName('yazıtura').setDescription('Bot ile yazı tura').addStringOption(o => o.setName('tahmin').setDescription('tahmin').setRequired(true).addChoices({ name: 'Yazı', value: 'yazi' }, { name: 'Tura', value: 'tura' })).addIntegerOption(o => o.setName('miktar').setDescription('Bahis').setRequired(true).setMinValue(1)))
  .addSubcommand(s => s.setName('bahis').setDescription('Zar bahsi: 1-49 kaybet, 50-99 x2, 100 x10').addIntegerOption(o => o.setName('miktar').setDescription('Bahis').setRequired(true).setMinValue(1)))
  .addSubcommand(s => s.setName('duel').setDescription('Üye ile R$ düellosu').addUserOption(o => o.setName('uye').setDescription('Rakip').setRequired(true)).addIntegerOption(o => o.setName('miktar').setDescription('Bahis').setRequired(true).setMinValue(1)))
  ...
  .addSubcommand(s => s.setName('market').setDescription('Mağazayı aç'))
  .addSubcommand(s => s.setName('envanter').setDescription('Envanterini gör'))
  .addSubcommand(s => s.setName('liderler').setDescription('Top 10 zengin'))
  .addSubcommand(s => s.setName('bilgi').setDescription('Ekonomi rehberi'));
CMDS.push({
  data: rb,
  async execute(i) {
    const sub = i.options.getSubcommand();
    const u = uconf(i.user.id);
    const bal = (x) => `**R$ ${x.toLocaleString('tr-TR')}**`;
    if (sub === 'bakiye') {
      const t = i.options.getUser('uye') || i.user;
      const tu = uconf(t.id);
      const eb = E(0x57F287).setTitle('💸 BAKİYE KARTI').setThumbnail(t.displayAvatarURL())
        .setDescription(`**Üye:** ${t.tag}\n**Cüzdan:** ${bal(tu.balance)}\n**Banka:** ${bal(tu.bank)}\n**Envanter:** ${tu.inv.length} eşya${tu.badges.includes('vip') ? '\n**Rozet:** 💎 VIP' : ''}`);
    ...
    if (sub === 'cash') {
      ...
      if (u.balance < m) return i.reply({ embeds: [ERR(`Yetersiz bakiye. Cüzdan: ${bal(u.balance)}`)], ephemeral: true });
      u.balance -= m; uconf(t.id).balance += m;
      dmUser(t, E(0x57F287).setDescription(`> **💸 TRANSFER**\n**Gönderen:** ${i.user.tag}\n**Miktar:** ${bal(m)}`));
      return i.reply({ embeds: [OKC(`${t.tag} üyesine ${bal(m)} gönderildi.`)] });
    }
    if (sub === 'günlük') {
      if (u.cd.daily > now()) return i.reply({ embeds: [ERR(`Günlük ödül için kalan: **${fmtDur(u.cd.daily - now())}**`)], ephemeral: true });
      let kaz = 250 * boostMul(i.user);
      u.balance += kaz; u.cd.daily = now() + 86400000;
      return i.reply({ embeds: [OKC(`Günlük ödül: ${bal(kaz)}${boostMul(i.user) > 1 ? ' *(2x boost aktif ✨)*' : ''}`)] });
    }
    if (sub === 'çalış') {
      if (u.cd.work > now()) return i.reply({ embeds: [ERR(`Tekrar çalışmak için: **${fmtDur(u.cd.work - now())}**`)], ephemeral: true });
      const job = pick(JOBS); let kaz = rnd(100, 400) * boostMul(i.user);
      u.balance += kaz; u.cd.work = now() + 3600000;
      return i.reply({ embeds: [OKC(`**${job}** olarak çalıştın ve ${bal(kaz)} kazandın.`)] });
    }
    if (sub === 'ara') {
      if (u.cd.ara > now()) return i.reply({ embeds: [ERR(`Tekrar aramak için: **${fmtDur(u.cd.ara - now())}**`)], ephemeral: true });
      let kaz;
      const sansIdx = u.inv.findIndex(x => x.id === 'sans');
      if (sansIdx > -1) { kaz = rnd(150, 300); u.inv.splice(sansIdx, 1); }
      else kaz = rnd(15, 120) * boostMul(i.user);
      u.balance += kaz; u.cd.ara = now() + 1800000;
      const extra = Math.random() < 0.05 ? (u.inv.push({ id: 'vip_parca' }), '\n> 💎 Nadir parça buldun! (envantere eklendi)') : '';
      return i.reply({ embeds: [OKC(`Aramada ${bal(kaz)} buldun.${extra}`)] });
    }
    if (sub === 'yazıtura') {
      ...
      if (sonuc === t) { u.balance += m; return i.reply({ embeds: [OKC(`Yazı tura: **${sonuc}** → Kazandın ${bal(m)}!`)] }); }
      u.balance -= m;
      return i.reply({ embeds: [ERR(`Yazı tura: **${sonuc}** → Kaybettin ${bal(m)}.`)] });
    }
    if (sub === 'bahis') {
      const m = i.options.getInteger('miktar');
      if (u.balance < m) return i.reply({ embeds: [ERR('Yetersiz bakiye.')], ephemeral: true });
      const r = rnd(1, 100);
      if (r === 100) { u.balance += m * 9; return i.reply({ embeds: [OKC(`✨ JACKPOT! Zar: **${r}** → x10 kazandın (${bal(m * 10)})`)] }); }
      if (r >= 50) { u.balance += m; return i.reply({ embeds: [OKC(`Zar: **${r}** → x2 kazandın (${bal(m * 2)})`)] }); }
      u.balance -= m;
      return i.reply({ embeds: [ERR(`Zar: **${r}** → Kaybettin ${bal(m)}.`)] });
    }
    if (sub === 'duel') {
      ...
      if (u.balance < m || uconf(t.id).balance < m) return i.reply({ embeds: [ERR('Senin veya rakibin bakiyesi yetersiz.')], ephemeral: true });
    ...
    if (sub === 'yatır') { const m = i.options.getInteger('miktar'); if (u.balance < m) return i.reply({ embeds: [ERR('Yetersiz bakiye.')], ephemeral: true }); u.balance -= m; u.bank += m; return i.reply({ embeds: [OKC(`Bankaya ${bal(m)} yatırıldı.`)] }); }
    if (sub === 'çek') { const m = i.options.getInteger('miktar'); if (u.bank < m) return i.reply({ embeds: [ERR('Bankada yeterli yok.')], ephemeral: true }); u.bank -= m; u.balance += m; return i.reply({ embeds: [OKC(`Bankadan ${bal(m)} çekildi.`)] }); }
    if (sub === 'market') {
      const rows = [];
      for (let x = 0; x < SHOP.length; x += 5) {
        rows.push(new ActionRowBuilder().addComponents(SHOP.slice(x, x + 5).map(it =>
          new ButtonBuilder().setCustomId(`shop:buy:${it.id}`).setLabel(`🛒 ${it.name.split(' (')[0]} — R$ ${it.price}`).setStyle(ButtonStyle.Secondary))));
      }
      return i.reply({
        embeds: [E(0x5865F2).setTitle('🛒 ROBUX MARKET').setDescription(SHOP.map(it => `**${it.name}** — ${bal(it.price)}\n> ${it.desc}`).join('\n\n'))],
        components: rows, ephemeral: true
      });
    }
    if (sub === 'envanter') {
      return i.reply({ embeds: [E().setTitle('🛒 ENVANTER').setDescription(u.inv.length ? u.inv.map(x => { const it = SHOP.find(s => s.id === x.id); return `**${it ? it.name : x.id}**${x.until ? ` (kalan: ${fmtDur(x.until - now())})` : ''}`; }).join('\n') : '> Envanterin boş.')], ephemeral: true });
    }
    if (sub === 'liderler') {
      const top = Object.entries(DB.users).sort((a, b) => (b[1].balance + b[1].bank) - (a[1].balance + a[1].bank)).slice(0, 10);
      return i.reply({ embeds: [E(0xFEE75C).setTitle('📊 ROBUX LİDERLERİ').setDescription(top.map((t, x) => `**${x + 1}.** <@${t[0]}> — ${bal(t[1].balance + t[1].bank)}`).join('\n') || '> Veri yok.')] });
    }
    if (sub === 'bilgi') {
      return i.reply({ embeds: [E(0x57F287).setTitle('💸 ROBUX EKONOMİ REHBERİ').setDescription(
        '> **Para birimi:** Robux (R$)\n\n**Kazanma yolları:**\n`/robux günlük` → 24 saatte bir 250 R$\n`/robux çalış` → saatlik iş kazancı\n`/robux ara` → 30 dk\'da bir rastgele R$ (+%5 nadir 💎)\n\n**Oyunlar:**\n`/robux yazıtura` → bot ile coinflip\n`/robux bahis` → zar: 1-49 kaybet, 50-99 x2, 100 x10\n`/robux duel` → üye ile düello\n\n**Banka & Market:**\n`/robux yatır` `/robux çek` → güvenli banka\n`/robux market` → boost, tılsım, VIP rozet\n`/robux cash` → üyeler arası transfer\n\n-# Tüm kazançlar globaldir, her sunucuda geçerli.')] });
    }
  }
});
...
/* ---------- GENEL ---------- */
CMDS.push({
  data: new SlashCommandBuilder().setName('ping').setDescription('🌐 Bot gecikmesi'),
  async execute(i) { i.reply({ embeds: [E().setDescription(`> **🌐 PONG**\nGateway: **${client.ws.ping}ms**`)] }); }
});
CMDS.push({
  ...
  async execute(i) {
    const t = i.options.getUser('uye') || i.user;
    i.reply({ embeds: [E().setTitle(`🪪 ${t.tag}`).setImage(t.displayAvatarURL({ size: 512 }))] });
  }
});
CMDS.push({
  data: new SlashCommandBuilder().setName('sunucu-bilgi').setDescription('🌐 Sunucu istatistikleri').setDMPermission(false),
  async execute(i) {
    const g = i.guild;
    const bots = g.members.cache.filter(m => m.user.bot).size;
    i.reply({ embeds: [E().setTitle('🌐 SUNUCU BİLGİSİ').setThumbnail(g.iconURL() || null).setDescription(
...
CMDS.push({
  ...
  async execute(i) {
    ...
    i.reply({ embeds: [E().setTitle('🪪 ÜYE KARTI').setThumbnail(t.user.displayAvatarURL()).setDescription(
...
async function createTicket(i, reasonLabel) {
  ...
  const ch = await i.guild.channels.create({
    ...
    type: ChannelType.GuildText, parent: cat.id, topic: `Ticket: ${i.user.tag} | ${reasonLabel}`,
    permissionOverwrites: perms
  });
  DB.tickets[ch.id] = { guild: i.guild.id, owner: i.user.id, reason: reasonLabel, created: now(), closed: false };
  const eb = E(0x5865F2).setTitle('🛠️ TICKET').setDescription(`${conf.text || 'Yetkililer en kısa sürede ilgilenecek.'}\n\n**Sebep:** ${reasonLabel}\n**Sahip:** ${i.user}`);
  if (conf.thumb) eb.setThumbnail(conf.thumb);
  const content = [i.user.toString(), ...(conf.roles || []).map(r => `<@&${r}>`)].join(' ');
  await ch.send({
    content,
    embeds: [eb],
    components: [new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('tk:kapat').setLabel('Ticketi Kapat').setStyle(ButtonStyle.Danger))]
  });
  ...
  try {
    ...
    if (i.isButton()) {
      ...
      if (id === 'mq:kur') {
        ...
        const eb = E(0x5865F2).setTitle('🛠️ MEQUEEN STUDIO — HELP & SUPPORT').setDescription(MEQUEEN_TEXT);
        if (CONFIG.mequeenBanner && CONFIG.mequeenBanner.startsWith('http')) eb.setThumbnail(CONFIG.mequeenBanner);
        await ch.send({
          embeds: [eb],
          components: [new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('tk:secim').setPlaceholder('Ticket kategorisi seç...')
            .addOptions(MEQUEEN_CATS.map(c => ({ label: c, value: c }))))]
        });
      ...
      if (id === 'gw:info') {
        const g = DB.giveaways[i.message.embeds[0] ? i.message.id : i.message.id] || DB.giveaways[i.message.id];
      ...
      if (id === 'app:kur') {
        const st = STATE.get(sKey(i) + ':app');
        if (!st || !st.kanal || !st.log || !st.metin || !st.sorular.length)
          return i.update({ embeds: [ERR('Eksik: panel kanalı, log kanalı, metin ve en az 1 soru zorunlu.')], components: [] });
        const ch = i.guild.channels.cache.get(st.kanal);
        gconf(i.guild.id).app = { panel: st.kanal, log: st.log, rol: st.rol || null, metin: st.metin, sorular: st.sorular };
        await ch.send({
          embeds: [E(0x5865F2).setTitle(`📋 ${st.metin.baslik}`).setDescription(st.metin.acik)],
          components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('app:apply').setLabel('Başvuru Yap').setStyle(ButtonStyle.Primary))]
        });
      ...
      if (id.startsWith('app:view:')) {
        if (!adminCheck(i)) return i.reply({ embeds: [ERR('Bu başvurunun içeriğini sadece **Yönetici** görebilir. 🔑')], ephemeral: true });
      ...
      if (id.startsWith('app:reject:')) {
        ...
        const modal = new ModalBuilder().setCustomId(`app:rejectmod:${aid}`).setTitle('📋 Reddetme Açıklaması');
      ...
      if (id === 'dm:onay') {
        ...
        for (const u of targets) {
          const sent = await dmUser(u, { content: st.metin });
          sent ? okc++ : fail++;
      ...
      if (id.startsWith('duel:accept:') || id.startsWith('duel:decline:')) {
        ...
        let win = r1 === r2 ? null : (r1 > r2 ? d.from : d.to);
        if (win === d.from) { a.balance += d.amt; b.balance -= d.amt; }
        else if (win === d.to) { b.balance += d.amt; a.balance -= d.amt; }
        return i.update({
          embeds: [E(0xFEE75C).setTitle('💸 DÜELLO SONUCU').setDescription(`**<@${d.from}>:** ${r1}\n**<@${i.user.id}>:** ${r2}\n> ${win ? `Kazanan: <@${win}> (${balF(d.amt)})` : 'Berabere, bahis iade.'}`)],
          components: []
        });
      }
    ...
    if (i.isModalSubmit()) {
      ...
      if (id.startsWith('ann:modal:')) {
        ...
        const st = {
          exp: now() + 600000, type,
          baslik: i.fields.getTextInputValue('baslik'),
          icerik: i.fields.getTextInputValue('icerik'),
          thumb: i.fields.getTextInputValue('thumb') || null,
          image: i.fields.getTextInputValue('image') || null,
          renk: i.fields.getTextInputValue('renk') || null,
          kanal: i.channel ? i.channel.id : null, mention: 'yok', rol: null
        };
      ...
      if (id.startsWith('app:page:')) {
        ...
        return i.reply({ embeds: [OKC('Başvurun alındı! 📋 Sonuç DM üzerinden bildirilecek.')], ephemeral: true });
      }
      ...
      if (id === 'dm:uye') {
        const st = STATE.get(sKey(i) + ':dm');
        const raw = i.fields.getTextInputValue('uye');
        const mid = (raw.match(/\d{17,20}/) || [null])[0];
    ...
    console.error(e);
    try {
      if (i.deferred || i.replied) await i.editReply({ embeds: [ERR('Beklenmedik hata:\n`' + String(e.message).slice(0, 150) + '`')] });
      else await i.reply({ embeds: [ERR('Beklenmedik hata:\n`' + String(e.message).slice(0, 150) + '`')], ephemeral: true });
    } catch (e2) {}
  }
});
function balF(x) { return `**R$ ${x.toLocaleString('tr-TR')}**`; }
...
async function finalizeTicket(i, st, cats) {
  ...
  const eb = E(0x5865F2).setTitle('🛠️ HELP & SUPPORT').setDescription(st.metin);
  if (st.thumb && st.thumb.startsWith('http')) eb.setThumbnail(st.thumb);
  const comps = [];
  if (st.mode === 'kategorili' && cats.length) {
    comps.push(new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('tk:secim').setPlaceholder('Ticket kategorisi seç...').addOptions(cats.map(c => ({ label: c, value: c })))));
  } else {
    comps.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('tkpanel:create').setLabel('Ticket Oluştur').setStyle(ButtonStyle.Primary)));
  }
...
async function annPreviewUpdate(i, first = false) {
  ...
  const rows = [
    ...
    new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('ann:mention').setPlaceholder('Etiket seç...').addOptions(
      { label: 'Etiket Yok', value: 'yok' }, { label: '@everyone', value: 'everyone' }, { label: '@here', value: 'here' }, { label: 'Rol', value: 'rol' })),
    new ActionRowBuilder().addComponents(
...
function appPageModal(i, page) {
  ...
  qs.forEach((q, x) => modal.addComponents(new ActionRowBuilder().addComponents(
    new TextInputBuilder().setCustomId(`q${page * 5 + x}`).setLabel(q.slice(0, 45)).setStyle(TextInputStyle.Paragraph).setMaxLength(800).setRequired(true))));
  return i.showModal(modal);
}
async function appSetupUpdate(i) {
  ...
  if (st.sorular.length) {
    const btns = [];
    st.sorular.forEach((s, x) => btns.push(new ButtonBuilder().setCustomId(`app:sorusil:${x}`).setLabel(`✕ ${x + 1}. ${s.slice(0, 20)}`).setStyle(ButtonStyle.Danger)));
    for (let x = 0; x < btns.length; x += 5) rows.push(new ActionRowBuilder().addComponents(btns.slice(x, x + 5)));
  }
  return i.update({
    embeds: [E().setTitle('📋 Başvuru Kurulum').setDescription(
      `**Panel:** ${st.kanal ? `<#${st.kanal}>` : '❌'}\n**Log:** ${st.log ? `<#${st.log}>` : '❌'}\n**Kabul rolü:** ${st.rol ? `<@&${st.rol}>` : '—'}\n**Metin:** ${st.metin ? '✅' : '❌'}\n**Sorular (${st.sorular.length}):**\n${st.sorular.map((s, x) => `> \`{${x + 1}}\` ${s}`).join('\n') || '> yok'}`)],
    components: rows
  });
}
...
client.once('ready', async () => {
  ...
  for (const g of client.guilds.cache.values()) {
    await g.commands.set(CMDS.map(c => c.data)).catch(e => console.error('cmd set', e.message));
  }
  console.log(`[STUDIOBLOX] ${CMDS.length} komut ${client.guilds.cache.size} sunucuya kaydedildi.`);
});
client.on('guildCreate', async (g) => {
  await g.commands.set(CMDS.map(c => c.data)).catch(() => {});
});
...
setInterval(async () => {
  for (const [id, g] of Object.entries(DB.giveaways)) {
    ...
    /* geri sayım / katılımcı tazeleme */
    if (ch) {
      try {
        const msg = await ch.messages.fetch(g.mid);
        await msg.edit({ embeds: [gwEmbed(g)], components: [gwRow(g)] });
      } catch (e) {}
    }
  }
  saveDB();
