...
// --- 1. RENDER UPTIME İÇİN WEB SUNUCUSU ---
const app = express();
...
// --- 3. GEÇİCİ VERİTABANI VE SİSTEM HAFIZASI ---
...
// --- 4. TÜM SLASH KOMUTLARININ TANIMLARI ---
const commands = [
    ...
    new SlashCommandBuilder().setName('guncelleme-duyuru').setDescription('Güncelleme duyurusu yayınlar').addStringOption(o => o.setName('mesaj').setDescription('Duyuru içeriği').setRequired(true)).toJSON(),
    new SlashCommandBuilder().setName('leak-duyuru').setDescription('Leak/Sızıntı duyurusu yayınlar').addStringOption(o => o.setName('mesaj').setDescription('Sızıntı detayı').setRequired(true)).toJSON(),
    new SlashCommandBuilder().setName('dm-at').setDescription('Belirtilen kullanıcıya özelden mesaj atar').addUserOption(o => o.setName('hedef').setDescription('Kullanıcı').setRequired(true)).addStringOption(o => o.setName('mesaj').setDescription('Mesaj').setRequired(true)).toJSON(),

    // Genel Yardım
    new SlashCommandBuilder().setName('yardim').setDescription('Tüm bot komutlarını ve kategorilerini gösterir').toJSON()
];

// --- 5. READY EVENT & OTOMATİK KOMUT KAYDI ---
client.once('ready', async () => {
    ...
    try {
        ...
        console.error('[HATA] Slash komutları yüklenirken sorun oluştu:', error);
    }
});

// --- 6. GUILD MEMBER ADD (OTOROL VE SAYAÇ) ---
        ...
        if (commandName === 'yardim') {
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '📊 Sistemler', value: '`/sayac-kur`, `/guncelleme-duyuru`, `/leak-duyuru`, `/dm-at`' }
                )
                .setColor(0x5865F2);
            return interaction.reply({ embeds: [embed] });
        }
